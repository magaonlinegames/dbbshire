...
function LOG_USER_OUT(){
 ...
 function BANKTRANSFERPERMIT_LISTENER(){
  firebase.firestore().collection("BANKTRANSFERPERMIT").doc(MASTERACCOUNT)
  .onSnapshot(function(doc) {
    ...
    function getAccountOPP(document){
      ...
      docRef.get().then((doc) => {
          if (doc.exists) {
              ...
              if (MASTERACCOUNT == 'account1') {
                $('.old-transaction').addClass('hide');
                $('.new-transaction').removeClass('hide');

              }
              if (MASTERACCOUNT == 'account500') {
                $('#acc-lock').text('Account inactive since May 28, 2022');
                $('.lock_txt').text('Account inactive since May 28, 2022');
              }
